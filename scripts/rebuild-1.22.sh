...
main() {
    ...
    echo "  docker run --rm temporalio/server:${image_tag}-amd64 file /usr/local/bin/temporal-server"
    echo "  # Should show: x86-64"
    echo ""
    echo "  docker run --rm temporalio/server:${image_tag}-arm64 file /usr/local/bin/temporal-server"
    echo "  # Should show: ARM aarch64"
    echo ""
    echo "To verify the admin-tools images have correct binaries:"
    echo "  docker run --rm temporalio/admin-tools:${image_tag}-amd64 file /usr/local/bin/tctl"
    echo "  # Should show: x86-64"
    echo ""
    echo "  docker run --rm temporalio/admin-tools:${image_tag}-arm64 file /usr/local/bin/tctl"
    echo "  # Should show: ARM aarch64"
    echo ""
